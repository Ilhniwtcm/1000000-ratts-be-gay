...
function addRule(full_rule)
  ...
  elseif (getIndefinite(subject)) then
    subjectsToAdd=getMembers(subject,"list",subject_not%2==0)
    for _,j in ipairs(subjectsToAdd) do addRuleSimple({j, rules.subject.conds}, rules.verb, rules.object, units, dir) end
  elseif subject_not % 2 == 1 then
    if (getTile(subject) or subject == "txt") and not(getIndefinite(subject))then
  ...
  elseif (getIndefinite(object)) then
    objectsToAdd=getMembers(object,"list",object_not%2==0)
    for _,j in ipairs(objectsToAdd) do addRuleSimple(rules.subject, rules.verb, {j, rules.object.conds}, units, dir) end
  ...
  elseif object == "notranform" or (subject == "lvl" and object == "lvl") or object == "local" then -- no "n't" here, but still blocks other rules so we need to count it
    ...
    if not_rules[n] then
      for _,rules in ipairs(not_rules[n]) do
        ...
        elseif (rule.verb.name == "ben't")  and rule.object.name == rule.subject.name or rule.object.name == "notranform" then -- "bab be n't bab" and 'bab be n't notranform' should cross out "bab be bab" (bab be notranform)
          specialmatch = 2
        elseif rule.verb.name == "be" and rule.object.name == "local" then -- "bab be local should cross out bab be global"
          specialmatch = 3
        end
        ...
        local function blockRules(t)
          ...
          for _,frules in ipairs(t) do
            ...
            if specialmatch ~= 1 and specialmatch ~= 3 then
              fverb = fverb .. "n't"
            end
            ...
            if (frule.subject.name == rule.subject.name or (rule.subject.name == "txt" and frule.subject.name:starts("txt_"))) and fverb == rule.verb.name and (
              ...
              (specialmatch == 3 and frule.object.name == "global")
            ) then
              if has_conds then
