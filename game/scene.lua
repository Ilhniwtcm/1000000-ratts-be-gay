...
negativeShader = pcallNewShader[[
  vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    number g = texturecolor.g;
    number b = texturecolor.b;
    return vec4(1-r, 1-g, 1-b, texturecolor.a);
  }
]]
...
function scene.buildUI()
  ...
  if not options then
    ...
    scene.addButton("meow :3 ",function() local sound = love.sound.newSoundData("assets/audio/sfx/cat.wav");
      local source = love.audio.newSource(sound, "static")
      source:setVolume(1)
      source:setPitch(2^(math.random(-12,12)/12))
      source:play()end)
  else
    buildOptions()
  end
...
function scene.getTransform()
  ...
  if viewport then
    ...
    local rot = 3.141582/4*(viewport.rot - 1)
    x=-(viewport.x + viewport.w/2) * TILE_SIZE + scaledwidth/2
    y=-(viewport.y + viewport.h/2) * TILE_SIZE + scaledheight/2
