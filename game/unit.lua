...
function updateUnits(undoing, big_update)
  ...
  if (big_update and not undoing) then
    ...
    local isglobal = hasProperty(nil,"global")
    ...
    for _,ruleparent in ipairs(iscrash) do
      ...
      if not hasProperty(ruleparent[1].rule.object,"slep") then
        local stuff = getUnitsOnTile(unit.x, unit.y, {not_destroyed = true, checkmous = true, thicc = thicc_units[unit]})
        for _,on in ipairs(stuff) do
          if hasU(on) and sameFloat(unit, on) and ignoreCheck(on, unit, "delet") and not hasProperty(unit,"local") and not hasProperty(on,"local") and (not (isglobal) or (hasProperty(on,"global") or hasProperty(unit,"global"))) then
    ...
    for _,unit in ipairs(isbonus) do
      local stuff = getUnitsOnTile(unit.x, unit.y, {not_destroyed = true, checkmous = true, thicc = thicc_units[unit]})
      for _,on in ipairs(stuff) do
        if hasU(on) and sameFloat(unit, on) and ignoreCheck(on, unit, ":o") and not hasProperty(unit,"local") and not hasProperty(on,"local") and (not (isglobal) or (hasProperty(on,"global") or hasProperty(unit,"global"))) then
    ...
    for _,unit in ipairs(isbonus) do
      local stuff = getUnitsOnTile(unit.x, unit.y, {not_destroyed = true, checkmous = true, thicc = thicc_units[unit]})
      for _,on in ipairs(stuff) do
        if hasU(on) and sameFloat(unit, on) and ignoreCheck(on, unit, ":o") and not hasProperty(unit,"local") and not hasProperty(unit,"local") and (not (isglobal) or (hasProperty(on,"global") or hasProperty(unit,"global"))) then
          writeSaveFile(true, {"levels", level_filename, "bonus"})
    ...
    for _,unit in ipairs(isunwin) do
      local stuff = getUnitsOnTile(unit.x,unit.y, {not_destroyed = true, checkmous = true, thicc = thicc_units[unit]})
      for _,on in ipairs(stuff) do
        if hasU(on) and sameFloat(unit, on) and ignoreCheck(on, unit, "un:)") and not hasProperty(unit,"local") and not hasProperty(on,"local") and (not (isglobal) or (hasProperty(on,"global") or hasProperty(unit,"global")))  then
          if timecheck(unit,"be","d") and (timecheckUs(on)) then
            if unit.negative or on.negative then
              unwins = wins + 0.0001
            else
              unwins = wins + 1
            end
    ...
    for _,unit in ipairs(iswin) do
      local stuff = getUnitsOnTile(unit.x, unit.y, {not_destroyed = true, checkmous = true, thicc = thicc_units[unit]})
      for _,on in ipairs(stuff) do
        if hasU(on) and sameFloat(unit, on) and ignoreCheck(on, unit, ":)") and not hasProperty(unit,"local") and not hasProperty(on,"local") and (not (isglobal) or (hasProperty(on,"global") or hasProperty(unit,"global"))) then
          if timecheck(unit,"be",":)") and (timecheckUs(on)) then
            wins = wins + 1
          else
    ...
    local isnegative = getUnitsWithEffect("negativ")--Don't ask
    for _,unit in ipairs(isnegative) do
      if not unit.negative then
        table.insert(to_update_negative,unit)
        addUndo({"negative_update",unit.id})
      end
    end
    local antinegative = getUnitsWithEffect("anti negativ")
    for _,unit in ipairs(antinegative) do
    ...
    for _,unit in ipairs(isnxt) do
      local stuff = getUnitsOnTile(unit.x, unit.y, {not_destroyed = true, checkmous = true, thicc = thicc_units[unit]})
      for _,on in ipairs(stuff) do
        if hasU(on) and sameFloat(unit, on) and ignoreCheck(on, unit, "nxt") and not hasProperty(unit,"local") and not hasProperty(unit,"local") and (not (isglobal) or (hasProperty(on,"global") or hasProperty(unit,"global"))) then
  ...
  for _,unit in ipairs(to_update_negative) do
    unit.negative = not unit.negative
    removeFromTable(to_update_negative,unit)
  end
  deleteUnits(del_units,false)
...
function convertLevel()
  ...
  for _,match in ipairs(converts) do
    ...
    if not (hasProperty(outerlvl, "lvl") or hasProperty(outerlvl, "local") or hasProperty(outerlvl, "global") or hasProperty(outerlvl, "notranform") or hasProperty(match.rule.object.name, "local")) and object.type and (object.type.object or object.name:starts("txt_")) and object.name ~= "no1" and (not (isglobal) or hasProperty(match.rule.object.name, "global")) then
      if match.rule.object.name == "txt" then
      ...
      if tile == nil and match.rule.object.name == "every1" then
        for _,v in ipairs(referenced_objects) do
          if not (hasRule(outerlvl, "ben't", v) or hasRule (v,"be","local")) then
    ...
    if not unit.new and unit.type ~= "outerlvl" and timecheck(unit,"be","thingify") then
      ...
      for i = 1,amt do
        ...
        elseif nametocreate:starts("txt_") then
          ...
          if getIndefinite(newname) then
            local things=getMembers(newname,"list",true)
...
function createUnit(tile,x,y,dir,convert,id_,really_create_empty,prefix,anti_gone) --ugh
  ...
  unit.negative = (rules_with ~= nil and rules_with["negativ"] and hasProperty(unit,"negativ"))
