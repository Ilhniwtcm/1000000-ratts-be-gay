...
function parseRules(undoing)
  ...
  local function reparseRuleCounts()
    ...
    table.insert(result, #matchesRule(nil, "!be", "wurd"))
...
function addRule(full_rule)
  ...
  while subject:starts("!") do subject, subject_not = subject:sub(1, -4), subject_not + 1 end
  while verb:ends("n't")    do verb,       verb_not =    verb:sub(1, -4),    verb_not + 1 end
  while verb:starts("!")    do verb,       verb_not =    verb:sub(1, -4),    verb_not + 1 end
  while object:ends("n't")  do object,   object_not =  object:sub(1, -4),  object_not + 1 end
  while object:starts("!")  do object,   object_not =  object:sub(1, -4),  object_not + 1 end
  ...
  elseif subject.type.indefinite_pronoun then
    if subject.mode == "subevery2" then
      for _,v in ipairs(referenced_objects) do
        if subject_not % 2 == 1 then
          shouldskip=false
          for b in ipairs(subject.members) do
            if v:ends(b) then
              shouldskip=true
              break
            end
  ...
  if object == "every1" then
    if object_not % 2 == 1 then
      return
    elseif verb ~= "be" and verb ~= "ben't" and verb ~= "!be" then
  ...
  elseif object.type.indefinite_pronoun then
    if object.mode == "subevery2" then
      for _,v in ipairs(referenced_objects) do
        if object_not % 2 == 1 then
          shouldskip=false
          for b in ipairs(object.members) do
            if v:ends(b) then
              shouldskip=true
              break
            end
      ...
      for _,rules in ipairs(not_rules[n]) do
        ...
        for i=1,2 do
          for _,cond in ipairs(conds[i]) do
            ...
            if new_cond.name:ends("n't") or new_cond.name:starts("!") then
        ...
        if rule.verb.name == "be" and rule.object.name == "notranform" then -- "bab be bab" should cross out "bab be keek"
          specialmatch = 1
        elseif (rule.verb.name == "ben't" or rule.verb.name == "!be")  and rule.object.name == rule.subject.name or rule.object.name == "notranform" then -- "bab be n't bab" and 'bab be n't notranform' should cross out "bab be bab" (bab be notranform)
          specialmatch = 2
        elseif rule.verb.name == "be" and rule.object.name == "global" then -- "bab be global should cross out bab be local"
          specialmatch = 3
        end
        ...
        local function blockRules(t)
          ...
          for _,frules in ipairs(t) do
            ...
            if (frule.subject.name == rule.subject.name or (rule.subject.name == "txt" and frule.subject.name:starts("txt_"))) and fverb == rule.verb.name and (
              ...
              (specialmatch == 2 and frule.object.name == "notranform") or
              (specialmatch == 3 and frule.object.name == "local")
            ) then
              if has_conds then
    ...
    if (#subject_cond > 0 and (even_non_wurd or subject:starts("txt") or rules_effecting_names[subject])) then
      for _,cond in ipairs(subject_cond) do
        ...
        if (#params > 0) then
          ...
          if cond_name == "frenles" or cond_name == "frenlesn't" or cond_name == "!frenles" then
            should_parse_rules = true
            return true
          elseif (cond_name == "corekt" or cond_name == "corektn't" or cond_name == "!corekt"  or cond_name == "rong" or cond_name == "rongn't" or cond_name == "!rong" ) then
            --nothing
          else
