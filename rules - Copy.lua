...
function addRule(full_rule)
  ...
  elseif (getIndefinite(subject)) then
    i=getIndefinite(subject)
    local subjectsToAdd={}
    if i["mode"]=="subevery2" then
      if subject_not % 2 == 1 then
        for _,v in ipairs(referenced_objects) do
          if not(i["members"][v] or (indefinite_objects[v]))  then
            table.insert(subjectsToAdd,v)
          end
        end
        for _,v in ipairs(referenced_text) do
          if not(i["members"][removeTxtPref(v)] or (indefinite_objects[v])) then
            table.insert(subjectsToAdd,v)
          end
        ...
        for _,v in ipairs(referenced_text) do
          if i["members"][removeTxtPref(v)] then
            table.insert(subjectsToAdd,v)
          end
        end
      end
    elseif i["mode"]=="subevery1" then
      if subject_not % 2==1 then
        for _,v in ipairs(referenced_objects) do
          if not(i["members"][v] or indefinite_objects[v]) then
            table.insert(subjectsToAdd,v)
  ...
  elseif (getIndefinite(object)) then
    i=getIndefinite(object)
    if i["mode"]=="subevery2" then
      if object_not % 2 == 1 then
        for _,v in ipairs(referenced_objects) do
          if i["members"][v] or (indefinite_objects[v]) then
            break
          end
          addRuleSimple(rules.subject, rules.verb, {v, rules.object.conds}, units, dir)
        end
        for _,v in ipairs(referenced_text) do
          if i["members"][removeTxtPref(v)] or (indefinite_objects[v]) then
            break
          end
          addRuleSimple(rules.subject, rules.verb, {v, rules.object.conds}, units, dir)
        end
